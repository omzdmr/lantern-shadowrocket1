/* Lantern Free -> Shadowrocket real Lua node JSON generator v18 */
(function () {
  "use strict";

  var HOST = "connect.rom.miui.com";
  var SINGLE_PATH = "/lantern-node-v18.json";
  var ALL_PATH = "/lantern-nodes-v18.json";
  ...
  function concat() {
    var out = [];
    for (var i = 0; i < arguments.length; i++) for (var j = 0, a = arguments[i] || []; j < a.length; j++) out.push(a[j] & 255);
    return out;
  }
  ...
  function fieldString(fieldNo, value) { return value ? fieldBytes(fieldNo, utf8Encode(String(value))) : []; }
  ...
  function buildConfigRequest() {
    ...
    return concat(fieldMessage(1, clientInfo), fieldMessage(2, fieldMessage(2, timestamp)));
  }
  ...
  Reader.prototype.varint = function () {
    ...
    while (this.p < this.b.length) {
      var x = this.b[this.p++]; n += (x & 0x7f) * mul;
      if ((x & 0x80) === 0) return n;
      mul *= 128; if (++count > 9) throw new Error("varint too long");
    }
  ...
  function parseTLS(bytes) {
    ...
    readFields(bytes, function (r,no,wire) {
      if (no === 1 && wire === 2) { o.sessionState = parseState(r.bytes()); return true; }
  ...
  function parseProxyContainer(bytes) {
    var out = [];
    readFields(bytes, function (r,no,wire) { if (no === 1 && wire === 2) { out.push(parseProxy(r.bytes())); return true; } return false; });
    return out;
  }
  ...
  function compatibleNodes(cfg) {
    ...
    (cfg.proxies || []).forEach(function (p) {
      ...
      out.push({
        name:String(p.name || ""), addr:String(p.addr), port:String(p.port), sni:String(t.sni), tlsFrag:String(t.tlsFrag || ""),
  ...
  function fail(msg) {
    $done({response:{status:502,headers:{"Content-Type":"text/plain; charset=utf-8","Cache-Control":"no-store"},body:"Lantern v18 ERROR\n" + String(msg)}});
  }

  function fetchAndBuild(all) {
    var bodyBytes = buildConfigRequest();
    var opts = {
      url:CONFIG_URL, method:"POST", timeout:20, "binary-mode":true, body:new Uint8Array(bodyBytes).buffer,
      headers:{
    ...
    $httpClient.post(opts, function (error,response,data) {
      ...
      try {
        var nodes = compatibleNodes(parseConfigResponse(normalizeBinary(data,response)));
        if (!nodes.length) return fail("compatible TLS 1.2/c02b proxy bulunamadi");
        if (all) return respondJson(nodes.map(function(n,i){return nodeJson(n,i+1);}), "Lantern-All-v18.json");
        return respondJson(nodeJson(nodes[0],1), "Lantern-Auto-v18.json");
      } catch (e) { return fail("decode/build: " + (e && e.message ? e.message : String(e))); }
    });
  }
